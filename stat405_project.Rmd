...
To ensure identifiability, we fix $\beta_{\text{Zverev}} = 0$ as a reference player and assign $\beta_i \sim \mathcal{N}(0, 9)$ for all other players. Zverev is chosen as a consistently active player across the full 2014–2026 period, giving a stable anchor across all four time periods. The choice $\sigma = 3$ allows skill differences up to 3 (win probabilities >95%), accounting for substantial variation in skill while keeping extreme differences unlikely.
...
We impose $\sum_s \gamma_{i,s} = 0$ per player for identifiability, with hierarchical prior $\gamma_{i,s} \sim \mathcal{N}(0, \tau_\gamma^2)$, $\tau_\gamma \sim \text{HalfNormal}(1)$. The shared $\tau_\gamma$ regularises surface effects when data are sparse.
...
with $\beta_{i,1} \sim \mathcal{N}(0, 9)$ and $\beta_{1,t} = 0$ for all $t$. This allows gradual changes in player ability over time.
...
```
Surface effects recover interpretable patterns: \textbf{Nadal} shows the textbook "King of Clay" profile, with a strongly positive clay effect and negative hard effect; Federer exhibits his strongest effect on hard courts. His negative estimated effects on clay and grass are partly due to the sum-to-zero constraint: since one surface must offset the others, a strong positive hard-court effect forces relative reductions elsewhere. This does not imply poor performance on grass, where Federer is historically dominant, but reflects the model’s identifiability structure. \textbf{Djokovic} shows a mild preference for grass over clay; \textbf{Alcaraz} and \textbf{Sinner} show small, imprecise effects, consistent with their shorter careers offering less surface-specific data.
...
```
Both models appear **well-calibrated**: predicted probabilities closely track observed frequencies across the full 0–1 range, with both curves hugging the diagonal. The two models are nearly indistinguishable in calibration, suggesting that while surface effects and time-varying skill capture different aspects of player performance, neither meaningfully improves calibration over the other. However, stronger comparison would require out-of-sample posterior predictive evaluation. Calibration code is in Appendix C.

# 7 Discussion

\section{Discussion}

Our Bayesian framework recovers sensible player rankings, captures known surface-specific performance patterns, and produces career trajectories consistent with historical narratives. Both model extensions are well-calibrated in-sample, but neither clearly outperforms the other. This suggests that surface effects and time variation encode similar predictive information through different modeling assumptions.

\textbf{Limitations.} We discard set and game scores, which contain additional information about match dominance. Restricting to 50 players introduces selection bias, and players with shorter careers have wider credible intervals. The Bradley–Terry model assumes independence between matches, ignoring momentum effects and head-to-head dynamics. 

A key limitation arises in the time-varying model: the random-walk prior propagates skill backward in time. As a result, players such as Alcaraz and Sinner receive non-zero skill estimates in early periods before their careers began. This is unrealistic and reflects a limitation of the model structure. A more realistic approach would activate player skill only after their first observed match.

Finally, calibration is evaluated in-sample; future work should include out-of-sample validation. A natural extension would combine both surface and time effects, though this would require stronger regularization to avoid identifiability issues.
